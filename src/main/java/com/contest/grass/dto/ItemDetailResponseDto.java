...
public class ItemDetailResponseDto {
    ...
    // 공통 필드
    @Schema(description = "상품의 고유 ID", example = "123", required = true)
    ...
    @Schema(description = "검색된 가격 범위 (최소)", example = "1000.00", required = false)
    private Double minPrice;        // 검색된 가격 범위 (최소)

    @Schema(description = "검색된 가격 범위 (최대)", example = "2000.00", required = false)
